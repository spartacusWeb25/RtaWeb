...
def proximo_codigo_sindicatopatronal(*, banco: str, db_alias: str, empresa: int, filial: int) -> int:
...
class SindicatoPatronalService:
    ...
    def salvar_form(form, banco, db_alias, sind_empr=None, sind_fili=None, operacao=None, **kwargs):
        ...
        if operacao == "criar":
            if codi_i is None or codi_i <= 0:
                instance.sind_codi = proximo_codigo_sindicatopatronal(
                    banco=banco_limpo,
                    db_alias=db_alias,
                    empresa=empr_i,
                    filial=fili_i,
            ...
            qs_existente = SindicatosPatronais.objects
            if db_alias:
            ...
            if duplicado:
                raise ValidationError(
                    f"Já existe um Sindicato Patronal cadastrado com o Código {codi_i} "
